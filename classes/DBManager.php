...
class DBManager implements Database
{
    ...
    {
        $conn = mysqli_connect(ConfigurationManager::getDbSettings()["DB_HOST"], ConfigurationManager::getDbSettings()["DB_USER"], ConfigurationManager::getDbSettings()["DB_PASS"]);
        if ($conn)
        {
            $db_select = mysqli_select_db($conn, ConfigurationManager::getDbSettings()["DB_NAME"]);
            if ($db_select)
            {
                return true;
            }
    ...
    {
        $this->connection = mysqli_connect(ConfigurationManager::getDbSettings()["DB_HOST"], ConfigurationManager::getDbSettings()["DB_USER"], ConfigurationManager::getDbSettings()["DB_PASS"]);
        if ($this->connection)
        {
            $db_select = mysqli_select_db($this->connection, ConfigurationManager::getDbSettings()["DB_NAME"]);
            if ($db_select)
            {
                return true;
            }
    ...
    public function selectDatabase($database)
    {
        ...
        else
        {
            ...
            mysqli_select_db($this->connection, ConfigurationManager::getDbSettings()["DB_NAME"]);
        }
    }
